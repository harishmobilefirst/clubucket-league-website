  ...
  return (
    <div className="rounded-[10px] overflow-hidden" style={{ background: "var(--cb-surface-panel)", boxShadow: "0 2px 12px rgba(0,0,0,0.07)" }}>
      ...
      <div className="relative px-6 py-10">
        {teams.length > 0 ? (
          <>
            <button type="button" onClick={() => shift("left")} aria-label={t("divisions.scrollLeft")} className="absolute left-3 top-1/2 -translate-y-1/2 z-10 w-11 h-11 rounded-full flex items-center justify-center shadow-md transition-colors" style={{ backgroundColor: "var(--cb-brand-primary)", color: "var(--cb-text-inverse)" }} onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = "var(--cb-brand-accent)"; }} onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = "var(--cb-brand-primary)"; }}><ChevronLeft className="w-5 h-5" /></button>
            <button type="button" onClick={() => shift("right")} aria-label={t("divisions.scrollRight")} className="absolute right-3 top-1/2 -translate-y-1/2 z-10 w-11 h-11 rounded-full flex items-center justify-center shadow-md transition-colors" style={{ backgroundColor: "var(--cb-brand-primary)", color: "var(--cb-text-inverse)" }} onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = "var(--cb-brand-accent)"; }} onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = "var(--cb-brand-primary)"; }}><ChevronRight className="w-5 h-5" /></button>
